...
export class BookingComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (!this.event) this.router.navigate(["Home"]);
  ...
  updateSection() {
    ...
    this.selectedSection.seats.forEach(obj => {
      if (!obj.flag) this.seat.push(obj);
    });
  ...
  add() {
    ...
    this.seatAdded.push(
      new SeatLocation(this.selectedSection.name, this.selectedSeat.location)
    );

    // change seat status
    this.selectedSection.seats.find(
      x => x.location == this.selectedSeat.location
    ).flag = true;
  ...
  bookTicket() {
    if (this.seatAdded.length < 1) return;
